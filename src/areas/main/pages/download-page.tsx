    ...
    <div className="container mx-auto max-w-4xl p-6 pb-24">
      ...
      {queue.length === 0 ? (
        ...
        <ul className="space-y-3">
          {queue.map((q) => (
            <li key={q.id} className="rounded-xl border border-foreground/10 p-4">
              <div className="flex items-center justify-between gap-4">
                ...
                <div className="w-40 h-2 rounded bg-foreground/10 overflow-hidden">
                  <div className="h-full bg-foreground/60" style={{ width: `${q.progress}%` }} />
                </div>
              </div>
              ...
              <div className="mt-2 flex gap-2">
                {q.status === 'active' && (
                  <button
                    ...
                    className="px-3 py-1 rounded border border-foreground/15 text-sm"
                  >
                    Cancel Active
                  </button>
                )}

...
export const Slide: React.FC<SlideProps> = ({ onClose }) => {
  ...
  const linkClass = (path: string) =>
    `flex gap-4 py-2 px-4 items-center rounded-xl duration-300 transition-all hover:scale-105 ${
      isActive(path) ? 'bg-linear-to-r from-accent-foreground to-accent text-black scale-105' : ''
    }`;
  ...
  return (
    ...
    >
      <motion.main
        className="w-[400px] bg-background h-screen overflow-y-auto p-8 flex flex-col gap-6"

...
const mergeDistributions = (primary: Distribution[] | undefined, fallback: Distribution[] | undefined): Distribution[] =>
...
export default function GameDetail(): React.ReactElement {
    ...
    const availablePlatforms = useMemo(() => {
        ...
        const source = mergeDistributions(game?.distribution, metadata?.distribution as Distribution[] | undefined);
    ...
    const renderHardwareCard = (tab: PlatformTab) => {
        ...
        if (!Array.isArray(specs) || specs.length === 0) {
            return <p className="text-muted-foreground">No hardware requirements provided.</p>;
        }
            ...
            <main className="flex justify-center py-24">
                <div className="max-w-[720px] text-center">
                    ...
                    <p className="text-muted-foreground">
                        We could not find the game you are looking for. It might be unpublished or removed from the
                        catalog.
                    </p>
                </div>
            </main>
    ...
    return (
        <main className="flex justify-center duration-300">
            <div className="max-w-[1400px] w-full flex flex-col gap-10 duration-300 px-6 sm:px-8 md:px-12">
                {/* Hero */}
                <section className="relative w-full min-h-[320px] rounded-3xl overflow-hidden shadow-flat-lg">
                    <img src={bannerImage} alt={game.name} className="absolute inset-0 w-full h-full object-cover" />
                        ...
                        <div className="flex flex-col gap-4 md:w-3/5">
                            <div className="flex flex-wrap gap-2 text-sm">
                                {categories.map((item) => (
                                    <span
                                        key={item}
                                        className="bg-background/70 px-3 py-1 rounded-full border border-white/10 uppercase tracking-wide text-foreground/80 backdrop-blur-sm font-semibold"
                                    >
                                        {item}
                                    </span>
                                ))}
                            </div>
                            <h1 className="text-4xl md:text-5xl font-bold leading-tight text-card">
                                {game.name ?? 'PeridotVault Game'}
                            </h1>
                            <p className="text-card/80 line-clamp-3 text-base md:text-lg">
                                {game.description ?? metadata?.description ?? 'No description provided yet.'}
                            </p>
                        </div>
                        <div className="flex flex-col md:flex-row gap-4 items-end md:w-2/5 justify-end">
                            <PriceCoin amount={rawPrice} tokenCanister={tokenCanister} textSize="lg" />
                            <button
                                type="button"
                                className={`px-10 font-semibold py-3 rounded-md bg-accent text-card shadow-flat-sm transition ${
                                    buying ? 'opacity-60 cursor-not-allowed' : 'hover:shadow-flat-lg'
                                }`}
                                onClick={handleBuyClick}
                                disabled={buying}
                            >
                                {buying ? 'Processing…' : priceIsFree ? 'Get' : 'Buy Now'}
                            </button>
                            {purchaseState ? (
                                <span
                ...
                {/* Body */}
                <section className="grid gap-12 lg:grid-cols-[3fr_1.25fr]">
                    <div className="flex flex-col gap-12">
                        {/* Previews */}
                        <div className="rounded-2xl border border-border bg-card shadow-flat-sm overflow-hidden">
                            <CarouselPreview
                                items={
                                    previews.length
                                        ? previews
                        ...
                        {/* About */}
                        <div className="rounded-2xl border border-border bg-card shadow-flat-sm p-8 space-y-6">
                            ...
                            <table className="w-full text-sm md:text-base">
                                <tbody>
                                    <KRow
                                        label="Released"
                                        value={
                                            releaseDate ? new Date(releaseDate).toLocaleDateString() : undefined
                                        }
                                    />
                                    <KRow
                                        label="Website"
                        ...
                        <div className="rounded-2xl border border-border bg-card shadow-flat-sm p-8 space-y-6">
                            <div className="flex items-center justify-between">
                                <h2 className="text-2xl font-semibold">Hardware Requirements</h2>
                            ...
                            <div className="grid gap-4">
                                {activeTab ? renderHardwareCard(activeTab) : (
                                    <p className="text-muted-foreground">
                                        Hardware information will appear here once configured.
                                    </p>
                                )}
                            </div>
                        </div>
                    ...
                    <aside className="flex flex-col gap-8">
                        <div className="rounded-2xl border border-border bg-card shadow-flat-sm p-6 space-y-6">
                            ...
                            <div className="space-y-3 text-sm text-muted-foreground">
                                ...
                                <div className="flex items-center gap-2">
                                    <FontAwesomeIcon icon={faAngleRight} />
                                    <span>
                                        Total purchased: {game.totalPurchased?.toLocaleString() ?? '0'}
                                    </span>
                                </div>
                            </div>
                        </div>

...
import { faBrain, faCheck, faGlobe, faHeading, faMessage, faMoneyBill1Wave, faPersonCane, faCalendarDays, faThumbTack } from "@fortawesome/free-solid-svg-icons";
...
import { AppStatus, ManifestInterface, Preview, Distribution, OS, UpdateAppInterface, AppInterface } from "../../interfaces/app/AppInterface";
...
export default function UpdateApp() {
    ...
    function hydrateFromApp(a: AppInterface) {
        ...
        const pitems: MediaItem[] = prevs
            .map((p) => {
                ...
                if (!url) return null;
                return {
                    kind: p.kind && (p.kind as any).video !== undefined ? "video" : extLooksVideo(url) ? "video" : "image",
                    src: url,
                    alt: "preview",
                } as MediaItem;
            })
    ...
    function distributionsToPayload(): Distribution[] {
        ...
        (["windows", "macos", "linux"] as OSKey[]).forEach((osk) => {
            ...
            const osVariant: OS = osk === "windows" ? ({ windows: null } as any) : osk === "macos" ? ({ macos: null } as any) : ({ linux: null } as any);
            list.push({
                native: {
                    os: osVariant,
            ...
            <Box sx={{ width: "100%" }}>
                ...
                <CustomTabPanel value={value} index={0}>
                    <div>
                        ...
                        {toast.ok && <div className="rounded-lg border border-success text-success px-4 py-2">{toast.ok}</div>}
                        {toast.err && <div className="rounded-lg border border-danger text-danger px-4 py-2">{toast.err}</div>}
                        <div className="flex justify-start">
                            <button type="button" disabled={busy} className={isCreateAnnouncementFormDisplayed ? "shadow-flat-sm my-6 px-6 py-3 rounded-md bg-red-500/20" : "shadow-flat-sm my-6 px-6 py-3 rounded-md bg-green-500/20"} onClick={() => setIsCreateAnnouncementFormDisplayed(!isCreateAnnouncementFormDisplayed)}>
                        ...
                        <form onSubmit={onAnnouncementSubmit} className={isCreateAnnouncementFormDisplayed ? "container flex flex-col gap-8" : "hidden"}>
                            ...
                            <InputFieldComponent name="Headline" icon={faHeading} type="text" placeholder="Headline" value={headline} onChange={(e) => setHeadline((e.target as HTMLInputElement).value)} />
                            <InputFieldComponent name="Content" icon={faMessage} type="text" placeholder="Content" value={content} onChange={(e) => setContent((e.target as HTMLInputElement).value)} />
                            <PhotoFieldComponent title="Cover Image" imageUrl={announcementCoverImage} onChange={handleAnnouncementCoverChange} />
                            <DropDownComponent
                                name="status"
                                icon={faCheck}
                                placeholder="Status"
                                ...
                                onChange={(e) => setAnnouncementStatus((e.target as HTMLSelectElement).value as keyof AppStatus)}
                            />
                            <div className="flex items-center gap-2">
                                <input type="checkbox" id="pin-announcement" checked={isAnnouncementPinned} onChange={(e) => setIsAnnouncementPinned(e.target.checked)} />
                            ...
                            <div className="flex justify-end">
                                <button type="submit" disabled={busy} className={`shadow-flat-sm my-6 px-6 py-3 rounded-md ${busy ? "opacity-60 cursor-not-allowed" : ""}`}>
                                ...
                                <div key={index} className={item.pinned ? "bg-green-500/20 p-6 flex justify-between" : "bg-gray-600 p-6 flex justify-between"}>
                                    <div>
                                        <div className="flex content-center mb-8">
                                            ...
                                            <p className="text-xl capitalize mr-4">{item.status && typeof item.status === "object" ? Object.keys(item.status)[0] : ""}</p>
                ...
                <CustomTabPanel value={value} index={1}>
                    <form onSubmit={onSubmit} className="container flex flex-col gap-8">
                        ...
                        <div className="flex gap-12">
                            ...
                            <div className="flex flex-col gap-8 w-2/3">
                                <section className="flex flex-col gap-4">
                                    ...
                                    <InputFieldComponent name="title" icon={faHeading} type="text" placeholder="Title" value={title} onChange={(e) => setTitle((e.target as HTMLInputElement).value)} />
                                    <InputFieldComponent name="description" icon={faMessage} type="text" placeholder="Description" value={description} onChange={(e) => setDescription((e.target as HTMLInputElement).value)} />
                                    <div className="grid grid-cols-2 gap-4">
                                        <InputFieldComponent name="price" icon={faMoneyBill1Wave} type="number" placeholder="Price (Nat)" value={priceStr} onChange={(e) => setPriceStr((e.target as HTMLInputElement).value)} />
                                        <InputFieldComponent name="requiredAge" icon={faPersonCane} type="number" placeholder="Required Age (Nat)" value={requiredAgeStr} onChange={(e) => setRequiredAgeStr((e.target as HTMLInputElement).value)} />
                                    </div>

                                    <InputFieldComponent name="releaseDate" icon={faCalendarDays} type="date" placeholder="Release Date" value={releaseDateStr} onChange={(e) => setReleaseDateStr((e.target as HTMLInputElement).value)} />
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <DropDownComponent
                                            ...
                                            onChange={(e) => setStatusCode((e.target as HTMLSelectElement).value as keyof AppStatus)}
                                        />
                                        <MultiSelectComponent maxValue={3} placeholder="Category" selected={selectedCategories} options={categoryOptions} onChange={setSelectedCategories} />
                                    </div>

                                    {/* Tags */}
                                    <div className="flex items-center gap-3">
                                        <input className="shadow-sunken-sm rounded-lg px-3 py-2 bg-transparent w-64" placeholder="Add tag and press +" value={tagInput} onChange={(e) => setTagInput(e.target.value)} />
                                        <button type="button" className="px-3 py-2 rounded-md shadow-flat-sm hover:shadow-arise-sm" onClick={addTag}>
                                            +
                                        </button>
                                    </div>
                                    {appTags.length > 0 && (
                        ...
                        <section className="flex flex-col gap-8">
                            ...
                            <BannerFieldComponent title="Banner Image" imageUrl={bannerImage} onChange={handleBannerChange} />

                            <div className="w-full flex gap-4 justify-evenly">
                                <div className="w-1/2">
                                    <CarouselPreview items={previewItems} initialIndex={0} showThumbnails htmlElement={<input type="file" accept="image/*,video/*" onChange={handlePreviewUpload} className="h-20 aspect-video bg-transparent shadow-sunken-sm rounded-lg" />} />
                                </div>
                                <div className="w-1/3">
                                    <PhotoFieldComponent title="Cover Image" imageUrl={coverImage} onChange={handleCoverChange} />
                                </div>
                            </div>
                        </section>
                        ...
                        {hasDist(selectedDistribution, "web") && (
                            <section className="flex flex-col gap-4">
                                ...
                                <InputFieldComponent name="webUrl" icon={faGlobe} type="text" placeholder="https://your-app.example/play" value={webUrl} onChange={(e) => setWebUrl((e.target as HTMLInputElement).value)} />
                            </section>
                        )}
                        ...
                        {(hasDist(selectedDistribution, "windows") || hasDist(selectedDistribution, "macos") || hasDist(selectedDistribution, "linux")) && (
                            <section className="flex flex-col gap-4">
                                <h3 className="text-xl font-semibold pt-2">Native Hardware</h3>
                                <div className="grid grid-cols-2 gap-4">
                                    <InputFieldComponent name="processor" icon={faBrain} type="text" placeholder="Processor" value={processor} onChange={(e) => setProcessor((e.target as HTMLInputElement).value)} />
                                    <InputFieldComponent name="memory" icon={faBrain} type="number" placeholder="Memory (Nat)" value={memoryStr} onChange={(e) => setMemoryStr((e.target as HTMLInputElement).value)} />
                                    <InputFieldComponent name="storage" icon={faBrain} type="number" placeholder="Storage (Nat)" value={storageStr} onChange={(e) => setStorageStr((e.target as HTMLInputElement).value)} />
                                    <InputFieldComponent name="graphics" icon={faBrain} type="text" placeholder="Graphics" value={graphics} onChange={(e) => setGraphics((e.target as HTMLInputElement).value)} />
                                </div>
                                <InputFieldComponent name="notes" icon={faBrain} type="text" placeholder="Additional Notes" value={notes} onChange={(e) => setNotes((e.target as HTMLInputElement).value)} />
                            </section>
                        )}
...
function ManifestList({ osKey, items, onChange, onAdd, onRemove, onUploadFile }: { osKey: OSKey; items: ManifestInterface[]; onChange: (idx: number, field: keyof ManifestInterface, value: string) => void; onAdd: () => void; onRemove: (idx: number) => void; onUploadFile: (idx: number, file: File) => void }) {
    const fileInputs = useRef<Record<number, HTMLInputElement | null>>({});

    return (
        <div className="space-y-4">
            {items.map((m, i) => (
                <div key={`${osKey}-${i}`} className="grid grid-cols-2 gap-4 p-4 rounded-xl bg-background_secondary">
                    <input className="shadow-sunken-sm rounded-lg px-3 py-2 bg-transparent" placeholder="Version (e.g. 1.0.0)" value={m.version} onChange={(e) => onChange(i, "version", e.target.value)} />
                    <div className="flex items-center gap-2">
                        <button type="button" className="px-3 py-2 rounded-md shadow-flat-sm hover:shadow-arise-sm flex items-center gap-2" onClick={() => fileInputs.current[i]?.click()}>
                    ...
                    <input className="shadow-sunken-sm rounded-lg px-3 py-2 bg-transparent" placeholder="Bucket" value={m.bucket} onChange={(e) => onChange(i, "bucket", e.target.value)} />
                    <input className="shadow-sunken-sm rounded-lg px-3 py-2 bg-transparent" placeholder="Base Path" value={m.basePath} onChange={(e) => onChange(i, "basePath", e.target.value)} />
                    <input className="shadow-sunken-sm rounded-lg px-3 py-2 bg-transparent" placeholder="Checksum (sha256)" value={m.checksum} onChange={(e) => onChange(i, "checksum", e.target.value)} />
                    <input className="shadow-sunken-sm rounded-lg px-3 py-2 bg-transparent" placeholder="Content / filename" value={m.content} onChange={(e) => onChange(i, "content", e.target.value)} />
                    <input className="shadow-sunken-sm rounded-lg px-3 py-2 bg-transparent" placeholder="Size (MB)" type="number" value={Number.isFinite(m.size) ? String(m.size) : ""} onChange={(e) => onChange(i, "size", e.target.value)} />

                    <div className="col-span-2 flex justify-end">
                        <button type="button" onClick={() => onRemove(i)} className="px-3 py-1 rounded-md border border-danger text-danger hover:bg-danger/10">
                            Remove
                        </button>
                    </div>
                </div>
            ...
            <button type="button" onClick={onAdd} className="px-4 py-2 rounded-md shadow-flat-sm hover:shadow-arise-sm">
                + Add Manifest
            </button>
        </div>
    );

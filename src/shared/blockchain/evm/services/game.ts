...
const REGISTRY_ADDRESS = (import.meta.env.VITE_EVM_REGISTRY_ADDRESS || '0x2091278674cec58296f4e7b868c2c84f5942abad') as `0x${string}`;
const RPC_URL = import.meta.env.VITE_EVM_RPC_URL || 'https://base-sepolia.g.alchemy.com/v2/1DVQw8E8nb2dYnSH0GwPm';
        ...
        const finalGames = await Promise.all(
            ownedGames.map(async (owned) => {
                try {
                    ...
                    console.warn(`Failed to fetch metadata for ${owned.gameId}`, err);
                    return null;
                }
            }),
        );
...
export async function getGameByGameId({ gameId }: { gameId: string }): Promise<PGCGame> {
    ...
    const local = await (await import('@features/library/services/localDb')).libraryService.getById(gameId as any);
    if (local) {
        return {
            gameId: local.gameId,
            name: local.gameName,
...
export async function getPublishedGames(params: { start: number; limit: number }): Promise<PGCGame[]> {
    return [];
}
